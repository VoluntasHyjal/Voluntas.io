...
export const getPlayerProfileUrl = (player, token) => `${BNET_HOST}/${player}?namespace=profile-${REGION}&locale=${LOCALE}&access_token=${token}`
export const getPlayerAvatarUrl = (player, token) => `${BNET_HOST}/${player}/character-media?namespace=profile-${REGION}&locale=${LOCALE}&access_token=${token}`
export const getPlayerEquipmentUrl = (player, token) => `${BNET_HOST}/${player}/equipment?namespace=profile-${REGION}&locale=${LOCALE}&access_token=${token}`
...
export const jsonFetch = (url, params = { method: 'GET' }) => fetch(url, params).then(response => response.json())
//.catch(err => alert(err))
